...
BitDuration=2 #a bit duration as a funtion of the data generator internal bit duration
...
CNFGSIZE2=(MAXROWS+176)   # Number of bit per column (or double columns). This is will be use for the main pixel column but also for the dac programming
CNFGSIZE3=CNFGSIZE1
CNFGSIZE4=CNFGSIZE0
...
class DgeneDriver:
    ...
    as possible, by setting up the blocks and instruction sequences. The idea 
    is to take shortcuts whenever the are safe.
    ...
    def readout_single_pixel(self, row, clone=True):
        ...
        outfile = open('occupancy_scan.csv', 'w')
        outfile.write(program_column(','.join(['0' if i != row else '1' for i in xrange(MAXROWS)])))
    ...
    def enable_count_clock(self, freq=50):
        ...
        """
        if freq > 50:
            print "Maximum frequency is 50 Mz."
            freq = 50
        if freq < 1e-3:
